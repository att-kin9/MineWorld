...
{
	jmethodID showFullscreenAdId = jnienv->GetMethodID(nativeActivity,"showFullScreenAd","()V");

	if (showFullscreenAdId == 0) {
		assert("porting::copyAssets unable to find copy assets method" == 0);
	}

	jnienv->CallVoidMethod(app_global->activity->clazz, showFullscreenAdId);
}
...
void initAndroid()
{
	...
	nativeActivity = findClass("com/k1n9/minetest/MtNativeActivity");
	if (nativeActivity == 0) {
		errorstream <<
